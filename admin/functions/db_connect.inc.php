<?php
    ...
    if (!($id = mysqli_connect($servidor, $usuario, $password))) {
    ...
    {
        $sanitizeRules = ['FROM', 'SELECT', 'INSERT', 'DELETE', 'WHERE', 'DROP TABLE', 'SHOW TABLES'];
        foreach ($Target as $key => $value):
            if (is_array($value)): $arraSanitized[$key] = _antiSqlInjection($value); else:
                //$arraSanitized[$key] = (!get_magic_quotes_gpc()) ? addslashes(str_ireplace($sanitizeRules, '', $value)) : str_ireplace($sanitizeRules, '', $value);
                $arraSanitized[$key] = addslashes(str_ireplace($sanitizeRules, '', $value));
        endif;
        endforeach;
