<?php
    ...
    class Funcoes
    {
        ...
        public function gerarMenu($paginas, $url_painel)
        {
            ...
            if (isset($pagina_atual->operacoes)):
                                        foreach ($pagina_atual->operacoes as $operacao_atual):
                                            if ($operacao_atual == 'listar') {
                                                $icone = '<i class="entypo-list"></i>';
                                            }
            if ($operacao_atual == 'inserir') {
                $icone = '<i class="entypo-list-add"></i>';
            }
        ...
        public function gerarLista($campos, $campoID, $tabela, $modulo, $camposPagina)
        {
            ...
            $tabela_gerada .= '<tbody>';
            $sql = mysqli_query($_SG['link'], "SELECT * FROM $tabela ORDER BY $campoID DESC");
            ...
            switch ($campos_b):
                            case 'acao':
                                foreach ($campo_b as $campos_a => $campo_a):
                                    if ($campo_a == 'editar'):
                                        $tabela_gerada .= '<a href="?m='.$modulo.'&t=editar&id='.$row[$campoID].'" title="Editar" class="btn_acoes btn btn-blue btn-sm btn-icon icon-left"><i class="entypo-pencil"></i>Editar</a>'; elseif ($campo_a == 'deletar'):
                                        $tabela_gerada .= '<a href="?m='.$modulo.'&t=deletar&id='.$row[$campoID].'" title="Deletar" class="btn_acoes btn btn-red btn-sm btn-icon icon-left"><i class="entypo-cancel"></i>Deletar</a>'; elseif ($campo_a == 'mail'):
            ...
            //echo $query_c.'<br><br>';
            $sql_c = mysqli_query($_SG['link'], $query_c);
            $row_c = mysqli_fetch_array($sql_c);
            $tabela_gerada .= $row_c[$campo_b[1]];
            break;
            endswitch;
            $tabela_gerada .= '</td>'; else:
                        if (isset($camposPagina[$campo_b][1]) && $camposPagina[$campo_b][1] == 'upload-img'):
                            if ($row[$campo_b] != ''):
                                $tabela_gerada .= '<td style="text-align:center;"><img src="'.((substr($row[$campo_b], 0, 3) == 'htt') ? $row[$campo_b] : $camposPagina[$campo_b][2].$row[$campo_b]).'" height="40" title="'.$camposPagina[$campo_b][2].$row[$campo_b].'"></td>'; else:
                                $tabela_gerada .= '<td>[Sem imagem]</td>';
            endif; elseif (isset($camposPagina[$campo_b][1]) && $camposPagina[$campo_b][1] == 'date'):
                            $tabela_gerada .= '<td>'.$this->desconverteData($row[$campo_b]).'</td>'; else:
        ...
        public function popularCampos($modulo, $campoID, $id)
        {
            ...
            $sql = mysqli_query($_SG['link'], "SELECT * FROM $modulo WHERE $campoID = '$id'");
            $row = mysqli_fetch_array($sql);
            //$retorno = $modulo.' - '.$campoID.' - '.$id;

            if ($modulo == 'adm_usuarios') {
                $row['senha'] = '';
            }
        ...
        public function gerarCampos($campos, $campo, $db)
        {
            ...
            if (isset($campo['dica']) && $campo['dica'] != '') {
                $dica = ' data-dica="'.$campo['dica'].'"';
            } else {
                $dica = '';
            }
            if (isset($campo['sufixo']) && $campo['sufixo'] != '') {
                $sufixo = ' data-sufixo="'.$campo['sufixo'].'"';
            } else {
                $sufixo = '';
            }
            ...
            case 'select':
                    if ($campo[5]):
                        $retorno .= '<select '.$dica.$sufixo.' multiple="multiple" name="'.$campos.'[]" class="form-control multi-select">'; else:
            ...
            $sql = mysqli_query($_SG['link'], "SELECT * FROM $valor");
            while ($row = mysqli_fetch_array($sql)):
                                if ($campo[6]):
                                    if ($titulo.'-'.$row[$campo[3][$index]] == $db[$campos]) {
                                        $select = 'selected';
                                    } else {
                                        $select = '';
                                    }
            $retorno .= '<option value="'.$titulo.'-'.$row[$campo[3][$index]].'" '.$select.'>'.$this->trataTexto($row[$campo[4][$index]], 60).' (#'.$row[$campo[3][$index]].')</option>'; else:
                                    if ($row[$campo[3][$index]] == $db[$campos]) {
                                        $select = 'selected';
                                    } else {
                                        $select = '';
                                    }
            ...
            $index = $index + 1;
            endforeach; else:
                        $sql = mysqli_query($_SG['link'], "SELECT * FROM $campo[2]");
            while ($row = mysqli_fetch_array($sql)):
                            if ($campo[5]):
                                if (in_array($row[$campo[3]], explode(',', $db[$campos]))) {
                                    $select = 'selected';
                                } else {
                                    $select = '';
                                } else:
                                if ($row[$campo[3]] == $db[$campos]) {
                                    $select = 'selected';
                                } else {
                                    $select = '';
                                }
            ...
            case 'upload-img':
                    if ($db[$campos] == '' || isset($_GET[$campos])) {
                        ...
                        $retorno .= '<img src="'.((substr($db[$campos], 0, 3) == 'htt') ? $db[$campos] : $campo[2].$db[$campos]).'" class="imagem_form" alt="">';
        ...
        public function reajustaImagem($pasta, $nome_imagem, $caminho_imagem, $large_width, $large_height, $thumb = false)
        {
            ...
            if ($ext == 'png') {
                $image = imagecreatefrompng($caminho_imagem);
            } else {
            ...
            if (!isset($large_width)) {
                $large_width = 9000;
            }
            if (!isset($large_height)) {
                $large_height = 9000;
            }
        ...
        public function uploadImagem($arquivo, $pasta, $width, $height)
        {
            if (!empty($arquivo['name'])) {
                ...
                if (!isset($error)) {
                    ...
                    if ($width && $height):
                        $imagem_redim = $this->reajustaImagem($pasta, $nome_imagem, $caminho_imagem, $width, $height); else:
        ...
        public function salvarDados($campos, $dados, $arquivos, $id, $db, $acao, $banco, $tabela)
        {
            ...
            if ($tabela == 'adm_usuarios' && ($acao == 'editar' || $acao == 'inserir')) {
            ...
            $objeto->valor_campos[$campo['thumb'][3]] = $valor_thumb; else:
                                        $valor = $this->uploadImagem($arquivos[$campos], $campo[2], $campo[3], $campo[4]);
            endif; else:
            ...
            endif; elseif (isset($dados[$campos])):
        ...
        public function printMSG($msg = null, $tipo = null)
        {
            if ($msg != null):
                switch ($tipo):
                    case 'erro':
        ...
        public function trataTexto($texto, $tamMax = 200)
        {
